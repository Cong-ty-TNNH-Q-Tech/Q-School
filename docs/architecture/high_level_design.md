...
- **Relational Database:** **PostgreSQL** (Lưu trữ User, Lớp, Học bạ, Cấu trúc bài tập).
- **Cache / Session DB:** **Redis** (Lưu trữ phiên đăng nhập, Token, và Caching kết quả).
- **Vector Database (Tùy chọn):** **Qdrant / Milvus** (Lưu trữ Embeddings để AI tra cứu sách giáo khoa qua RAG).
- **Object Storage:** **MinIO / AWS S3** (Chứa file ảnh, PDF, Word, Video).
