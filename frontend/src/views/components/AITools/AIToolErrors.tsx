...
export function PaymentRequiredBanner({ onUpgrade }: PaymentRequiredBannerProps) {
  return (
    <div className="bg-rose-50 border border-rose-200 rounded-xl p-4 mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 animate-in fade-in slide-in-from-top-2 duration-300">
...
export function RateLimitWarning({ secondsLeft }: RateLimitWarningProps) {
  ...
  return (
    <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 mb-6 flex items-center gap-3 animate-in fade-in slide-in-from-top-2 duration-300">
      <AlertCircle className="w-5 h-5 text-amber-600 shrink-0" />
      <p className="text-sm text-amber-800">
        Bạn đang gửi yêu cầu quá nhanh. Vui lòng thử lại sau <span className="font-bold">{secondsLeft}</span> giây.
      </p>
    </div>
  );
}
