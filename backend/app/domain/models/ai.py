...
class ChatMessage(Base, UUIDMixin, TimestampMixin):
    ...
    __table_args__ = (
        CheckConstraint(
            "sender_type IN ('user', 'ai')", name="ck_chat_messages_sender_type"
        ),
    )
...
class Document(Base, UUIDMixin, TimestampMixin, SoftDeleteMixin):
    ...
    __table_args__ = (
        CheckConstraint(
            "file_type IN ('pdf', 'docx', 'image')", name="ck_documents_file_type"
        ),
        CheckConstraint(
            "status IN ('pending', 'parsing', 'ready', 'error')",
    ...
    file_type: Mapped[str] = mapped_column(
        String(20), nullable=False, comment="pdf, docx, image"
    )
