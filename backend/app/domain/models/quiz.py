...
from sqlalchemy import String, Text, Boolean, Integer, Float, ForeignKey, DateTime, UniqueConstraint
...
class Quiz(Base, UUIDMixin, TimestampMixin, SoftDeleteMixin):
    ...
    content_source: Mapped[str | None] = mapped_column(Text, nullable=True, comment="Văn bản gốc dùng để AI sinh câu hỏi")
...
class Question(Base, UUIDMixin):
    ...
    media_url: Mapped[str | None] = mapped_column(String(512), nullable=True, comment="Ảnh/Video minh họa")
    explanation: Mapped[str | None] = mapped_column(Text, nullable=True, comment="Giải thích đáp án")
...
class EssaySubmission(Base, UUIDMixin, TimestampMixin):
    ...
    content: Mapped[str] = mapped_column(Text, nullable=False, comment="Nội dung bài văn")
    ...
    rubric: Mapped["Rubric | None"] = relationship("Rubric", back_populates="essay_submissions")
...
class StudentAnswer(Base, UUIDMixin):
    ...
    attempt: Mapped["QuizAttempt"] = relationship("QuizAttempt", back_populates="student_answers")
