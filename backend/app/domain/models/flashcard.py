...
class Flashcard(Base, UUIDMixin):
    ...
    media_url: Mapped[str | None] = mapped_column(
        String(512), nullable=True, comment="Ảnh minh họa"
    )

    set_: Mapped["FlashcardSet"] = relationship(
...
class FlashcardReview(Base, UUIDMixin):
    ...
    __table_args__ = (
        UniqueConstraint(
            "student_id", "flashcard_id", name="uq_flashcard_review_student_card"
        ),
    )
    ...
    student: Mapped["User"] = relationship(
        "User", foreign_keys=[student_id], viewonly=True
    )
